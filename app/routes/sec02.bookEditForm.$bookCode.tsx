  ...
  useEffect( ()=> {
    try {
        const fetchData = async () =>{
            const bookData =  await fetch('http://localhost:3000/api/editbook/' + myParam.bookCode );
        }
        fetchData();
    }catch (e){
    ...
    <>
      <div className="m-3">
        ...
        <form onSubmit={handleSubmit} method="POST">
          ...
          <button
            type="submit"
            className="me-2 text-white bg-yellow-500 hover:bg-yellow-800 focus:ring-4 focus:outline-none focus:ring-yellow-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-yellow-600 dark:hover:bg-yellow-700 dark:focus:ring-yellow-800"
          >
            บันทึก
          </button>
          <a href="/sec02/bookList"
            type="reset"
